...
const products: Product[] = [
  ...
  { id:'ceviche', category:'Ceviches', name:'Ceviche Nocturno', price:35, description:'Fresco, intenso y listo para la noche.' },
  { id:'leche', category:'Ceviches', name:'Leche de Tigre', price:25, description:'El golpe de sabor que manda.' },
  ...
  { id:'lomo', category:'Especiales', name:'Lomo Saltado', price:30, description:'Lomo, papas y arroz blanco.', image:'/lomo-saltado-pollo.jpg' },
  ...
  { id:'lomo-pobre', category:'Especiales', name:'Lomo Saltado a lo Pobre', price:35, description:'Lomo saltado, huevo, plátano y arroz blanco.' },
...
export default function Home() {
  ...
  return <main data-site-version="salchipapas-20260826-2152">
    ...
    <section className="hero" id="inicio"><div className="hero-copy"><span className="eyebrow">EL PATRÓN ESTÁ DE REGRESO EN HUACHO</span><h1>CUANDO EL HAMBRE<br/><em>MANDA,</em> EL PATRÓN<br/>RESPONDE.</h1><p>Las mejores hamburguesas, alitas o pollito broster lo tenemos nosotros. Atendemos desde las 11 a. m. hasta las 3 a. m.</p><div className="hero-actions"><a href="#carta" className="button primary">Ver la carta</a><a href="https://wa.me/51926304161" className="button ghost">WhatsApp · 926 304 161</a></div></div><div className="hero-art"><img src="/portada-el-patron.png" alt="Hamburguesa El Patrón de doble carne"/><span className="hero-stamp">SABOR<br/>QUE<br/>MANDA</span></div></section>
    ...
    {selectedProduct && <div className="modal-backdrop" role="presentation"><div className="extras-modal" role="dialog" aria-modal="true" aria-labelledby="extras-title"><button className="modal-close" onClick={() => setSelectedProduct(null)} aria-label="Cerrar">×</button><span className="eyebrow">{selectedProduct.category === 'Alitas' ? 'Elige una salsa' : selectedProduct.category === 'Salchipapas' ? 'Completa tu salchipapa' : selectedProduct.category === 'Pollo Broaster' || selectedProduct.category === 'Papas Nativas' ? 'Elige tus salsas' : 'Hazlo a tu manera'}</span><h2 id="extras-title">{selectedProduct.name}</h2><p>{selectedProduct.category === 'Alitas' ? 'Tu salsa viene incluida. Elige una para continuar.' : selectedProduct.category === 'Salchipapas' || selectedProduct.category === 'Pollo Broaster' || selectedProduct.category === 'Papas Nativas' ? 'Cuéntanos qué más quieres que le pongamos.' : selectedProduct.description}</p><div className="extra-list">{selectedProduct.category === 'Alitas' ? wingSauces.map(sauce => <label key={sauce}><input type="radio" name="wing-sauce" checked={selectedExtras[0] === sauce} onChange={() => setSelectedExtras([sauce])}/><span>{sauce}</span><b>Incluida</b></label>) : selectedProduct.category === 'Salchipapas' ? freeComplements.map(complement => <label key={complement}><input type="checkbox" checked={selectedExtras.includes(complement)} onChange={() => setSelectedExtras(current => current.includes(complement) ? current.filter(item => item !== complement) : [...current,complement])}/><span>{complement}</span><b>Incluido</b></label>) : selectedProduct.category === 'Pollo Broaster' || selectedProduct.category === 'Papas Nativas' ? broasterSauces.map(sauce => <label key={sauce}><input type="checkbox" checked={selectedExtras.includes(sauce)} onChange={() => setSelectedExtras(current => current.includes(sauce) ? current.filter(item => item !== sauce) : [...current,sauce])}/><span>{sauce}</span><b>Incluida</b></label>) : extras.map(extra => <label key={extra.name}><input type="checkbox" checked={selectedExtras.includes(extra.name)} onChange={() => setSelectedExtras(current => current.includes(extra.name) ? current.filter(item => item !== extra.name) : [...current,extra.name])}/><span>{extra.name}</span><b>+{money(extra.price)}</b></label>)}</div><button className="send-order" disabled={selectedProduct.category === 'Alitas' && !selectedExtras.length} onClick={() => addProduct(selectedProduct,selectedExtras)}>{selectedProduct.category === 'Alitas' && !selectedExtras.length ? 'Elige una salsa para continuar' : `Agregar al pedido · ${money(selectedProduct.price + selectedExtras.reduce((total,extra) => total + (extras.find(item => item.name === extra)?.price ?? 0),0))}`}</button></div></div>}
